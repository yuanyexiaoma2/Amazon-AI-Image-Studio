...
import { RedoWorkflowCommandsRequestSchema, makeApiError } from '@studio/contracts';
...
/** POST .../commands/redo — re-apply an undone batch's afterGraph. */
export async function POST(request: Request, context: Ctx) {
  ...
  const parsed = RedoWorkflowCommandsRequestSchema.safeParse(body);
  if (!parsed.success) {
    return NextResponse.json(
      makeApiError('VALIDATION_ERROR', 'Invalid redo payload', requestId, parsed.error.flatten()),
  ...
  try {
    const { draft } = await workflows.redoCommandBatch({
      workspaceId,
      workflowId,
