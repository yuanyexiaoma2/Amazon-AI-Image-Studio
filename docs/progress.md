...
| PR-6-13 | kie 模型更新（+nb2/lite/gpt2.5 −Seedream） | DONE | 所有者指定三个市场页（kie.ai/nano-banana-2、/nano-banana-2-lite、/gpt-image-2-5）接入并删除 Seedream 5 Pro。参数规格来自 docs.kie.ai 官方 OpenAPI。domain：删 `KIE_GENERATE_MODEL`/`KIE_EDIT_MODEL`（seedream）与 `KIE_EXTENDED_MODEL_REGISTRY`；新增 `KIE_NANO_BANANA_2_MODEL`（单 modelId 文/图合一，image_input ≤14，14 档比例含 1:4/4:1/1:8/8:1，1K/2K/4K，估 $0.06）、`KIE_NANO_BANANA_2_LITE_MODEL`（参考图参数是 **image_urls** ≤10，无 resolution/output_format 只出 1K，占位 $0.04）、GPT Image 2.5 四条目（Flare/Sunburst × t2i/i2i，共用 `GPT_IMAGE_2_5_BASE`；i2i 用 input_urls ≤16；登记比例剔除只支持 1K 的 27:16/16:27/9:8/8:9 及 auto，剩 8 档全兼容 1K/2K/4K；占位 $0.05）。注册表共 9 条 kie 条目；AUTO 偏好序改 nb2 → gpt2.5-flare → nb-pro → gpt2 → fake。kie-adapter：`buildModelInput` 加 nb2 族（同 nb-pro，prompt 上限 20000、image_input ≤14）、lite 族（image_urls、无 resolution/output_format）、gpt2.5 并入 gpt2 前缀分支；每模型 credits 表加 nb2 分档 8/12/18cr、lite 8cr、gpt2.5 10cr（占位）。db generation.ts kie defaultKey 改 `kie-nano-banana-2`。zh-labels：Nano Banana 2（谷歌）/ Nano Banana 2 Lite（谷歌·快速）/ GPT Image 2.5 Flare·Sunburst（文生图/图片编辑，sunburst 标「·精细」）。providers 82/82（+4 家族测试）、domain 154、contracts 24、db 集成 40/40、web 54、lint/tsc 三绿；重建 domain/providers/db dist + 重启 dev:local，curl model-registry 确认 9 条目无 seedream（commit `ec1d4b3`）。**待实测**：gpt2.5 与 nb2-lite 的 kie 官方单价（占位值已注释）；图生图参考图 URL 对 kie 服务器可达性（本地 MinIO presigned 是 localhost，失败则需接 kie File Upload API 中转） |
